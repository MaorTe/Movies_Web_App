...
const AddOrRemoveBtn = ({ id, poster, title, type, onButtonClick, homePageClassName }) => {
   ...
   const onPosterClick = () => {
      !isExist ? setIsExist(MyUtilFunc(id, poster, title, type)) : setIsExist(removeMovie());
      onButtonClick();
   };
   const removeMovie = () => {
      const tableData = JSON.parse(localStorage.getItem('tableData'));
   ...
   return homePageClassName ? (
      <button className={homePageClassName} onClick={() => onPosterClick()}>
